...
public class PageUtil {
  ...
  private int count;     // 전체 목록의 개수 (SELECT 문 실행)
  private int pageCount; // 전체 페이지의 개수 (total과 display에 의해서 계산되는 항목)
  ...
  public void setPaging(int page, int display, int total) {
    ...
    int pagePerBlock= 10;
    pageCount = (int)Math.ceil((double)total / display); // 1001/ 50으로 계산할경우 생략되는 부분이 있어서 double을 넣어서 소수점까지 나오게 만듬
